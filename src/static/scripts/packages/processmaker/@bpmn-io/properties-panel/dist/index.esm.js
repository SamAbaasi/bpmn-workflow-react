...
function Select(props) {
  ...
  const [hoveredValueIndex,setHoveredValueIndex] = useState()
  ...
  const handleChange = e => {
    ...
    if(e.target.innerHTML !== 'empty'){
      findVal = options.find(op => op.label == e.target.innerHTML).value.toString();
      findLabel = options.find(op => op.label == e.target.innerHTML).label;
    }else{
      findVal = null;
      findLabel = '';
    }
    setLocalValue(findLabel);
    onChange(findVal);
  ...
  const displayState = useMemo(() => {
    const ds = {};
    ds.displayCross =  value !== null && value !== undefined;
    ds.displayDropdown =  isDropdownExpanded && !isEscapeClosed;
    return ds;
  }, [disabled, isDropdownExpanded, isEscapeClosed, value]);
  ...
  // }, [isDropdownExpanded]);
  useEffect(() => {
      const findLabel = options.find(op => op.value == value)?.label;
      setLocalValue(findLabel);
  }, [value]);
  ...
  document.addEventListener('click', (e) => {
    ...
    if (ref?.current && ref?.current?.contains(e.target)) {
      isDropdownExpanded ? setIsDropdownExpanded(false):  setIsDropdownExpanded(true);
    }
  })
  const [filtedOptions, setFilterOptions] = useState(options)
  const filteredOptions = (val) => {
    if (val !== '') {
      if(apiAddressForSearch !== ''){
        searchInSelectboxWithApi(apiAddressForSearch,val)
        setTimeout(() => {
           const filtedOptions = selectboxList
      ...
      // const filtedOptions = apiAddressForSearch !== '' ? : options.filter(data => data.label.toLowerCase().includes(val.toLowerCase()))
      return setFilterOptions(filtedOptions)
    }else{
  ...
  return jsxs("div",{
    class: classnames('position-relative'),
    style:{
      'margin-top':'4px'
    },
    ref: dropdownRef,
    children:[
      ...
      jsx("div",{
        children:[
          jsx("input", {
            ref: ref,
            name: id,
            class: classnames('bio-properties-panel-multiselect','bio-properties-panel-select'),
            onChange: handleChange,
            readOnly:true,
            onFocus: onFocus,
            onBlur: onBlur,
      ...
      //   },
        // children:[
          ...
          //     }),
              ...
              //   children: displayState.displayDropdown ? jsx(ArrowIcon, {}) : jsx(ArrowIcon, {})
              // }),
        // ]
      // },
      // ),
      ...
      //   children: [
          displayState.displayDropdown && jsxs("div", {
            ...
            // tabIndex: -1,
            class: classnames('bio-properties-panel-multiselect-options-container','bio-properties-panel-select-options'),
            ...
            children: [
              ...
              ,
              filtedOptions.length > 0 && filtedOptions.map((option, idx) => {
                return jsx("div", {
                  // class: classNames('bio-properties-panel-multiselect-option', {
                  //   // 'focused': focusedValueIndex === i
                  // }),
                  class: classNames('bio-properties-panel-multiselect-option', localValue == option.label ? 'bio-properties-panel-multiselect-option-selected' : ''),
                  // style:{
                  //   background: hoveredValueIndex == idx&& ishover ? "white" : "none",
                  //   padding:" 0 4px",
                  //   borderRadius: "5px",
                  //   overflow: "hidden",
                  ...
                  onMouseOver: e => {
                    if(e.target.innerHTML == option.label || e.target.innerHTML == 'empty'){
                      setIshover(true)
                      setHoveredValueIndex(idx)
                    }
                  },
                  onMouseLeave: e => {
                    if(e.target.innerHTML != option.label){
                      setIshover(false)
                  }
                },
                  children: option.value == null ? "empty" : option.label
                });
              }),
  ...
  //     id: prefixId$3(id),
  //     name: id,
  //     class: "bio-properties-panel-input w-100",
  //     onChange: handleChange,
  //     readOnly:true,
  //     onFocus: onFocus,
  //     onBlur: onBlur,
...
function MultiSelectEntry(props) {
  ...
  useEffect(() => {
    ...
    if(value){
      ...
      let labels = options.filter(op => valuesList.includes(op.value.toString())).map(({label}) => label)
  ...
  let selectedValueShowed = [];
  let showedValueInInput = '';
  const handleChange = (e) => {
    let valueList = options.filter(op => e?.split(',').includes(op.label)).map(({value}) => value)
  ...
  const filteredOptions = (val) => {
    ...
    if (val !== '') {
      if(apiName && apiName != ''){
        searchInMultiselectWithApi(apiName,val)
        setTimeout(() => {
           const filtedOptions = list
           return setFilterOptions(filtedOptions)
         }, 1000);
      }else{
        const filterdOptions = options.filter(op => op?.label?.toLowerCase()?.includes(val?.toLowerCase()))
        return setFilterOptions(filterdOptions)
      }
    }else{
      return setFilterOptions(options)
    }
  ...
  const onSelectOption = (op) =>{
    if (localValue) {
      ...
      selectedValueShowed = [...prevValue]
    }
    const searchElIndex = selectedValueShowed.findIndex(val => val == op.label);
    if (searchElIndex > -1 && selectedValueShowed.length !== 0) {
      selectedValueShowed.splice(searchElIndex, 1)
    } else {
      selectedValueShowed.push(op.label)
    }
    showedValueInInput = selectedValueShowed.join(',')
    // selectedValue = options.filter(op => selectedValueShowed.includes(op.label)).map(({value}) => value)
 ...
 useEffect(() => {
   setFilterOptions(options);
   const valueList = localValue?.split(',')
   for (let i = 0; i < valueList?.length; i++) {
    const findIndexValueInOptions = options.findIndex(op => op.label == valueList[i])
    if(findIndexValueInOptions == -1){
      valueList.splice(i,1)
    }
   }
   const currectValue = valueList?.join(',')
   if(currectValue != localValue){
      setLocalValue(currectValue)
      handleChange(currectValue)
   }
   }, [options])
  ...
  return jsxs("div", {
    ...
    children: [
      ...
      jsx("div",{
        children:[
          jsx("input", {
            ...
            value: localValue,
            readOnly: true,
            class: classnames('bio-properties-panel-multiselect','bio-properties-panel-multiselect-2'),
          }, element),
          isShowClearIcon &&  jsx("i",{
            innerHTML:'&times;',
            style:{
      ...
      showOp && jsx("ul", {
        class: classnames('bio-properties-panel-multiselect-options-container','bio-properties-panel-multiselect-options-container-2'),
        ref:optionsRef,
        children:[
          jsx("input", {
            type: "text",
              ...
              children:[
                jsx("div",{
                  ...
                  children:[
                    jsx("input",{
                      type:'checkbox',
                      checked: localValue?.split(',').includes(op.label) ? true :false,
                    }),
                    jsx("span",{
                      children:[op.label],
                      style:{
...
async function searchInMultiselectWithApi(api,text){
  ...
  const tagRequest = await new Request(
    `${api}?q=${text}`,
    options
  );
  fetch(tagRequest)
...
let selectboxList = []
async function searchInSelectboxWithApi(api,text){
  const token = window.localStorage.getItem('ngx-app.current-user') || '';
  const myHeaders = new Headers();
  let CSRFToken = '';
  let match = document.cookie.match(new RegExp('(^| )' + name + '=([^;]+)'));
  ...
  fetch(tagRequest)
    ...
    .then((res) => {
      if (res) {
        ...
        res?.map((resp) => {
          selectboxList.push({
            value: resp.task_name,
            label: resp.task_name,
          });
        });
      }
    });
...
function isEdited$10(node) {
  return node && !!node.value;
}

// helpers /////////////////

function prefixId$10(id) {
  return `bio-properties-panel-${id}`;
}
